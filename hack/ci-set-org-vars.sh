...
validateSourceControl() {
    if [[ "gitlab github" != *"$CI_SOURCE_CONTROL"* ]]; then
...
validateBackend() {
    ...
    if [[ "gitlab github azure" != *"$CI_BACKEND"* ]]; then
...
azureGetValues() {
    ...
    echo $AZURE_TOKEN | az devops login --organization $AZURE_ORG_URL
